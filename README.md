# Voorstel IFJ — AI Onboardingsdag

HTML-slidedeck met een voorstel voor een praktische AI-onboardingsdag voor het commerciële team en content/marketing bij IFJ.

## Live bekijken

**https://cschoorl.github.io/Voorstel-IFJ/**

> Werkt de link niet (404)? Zet GitHub Pages aan:
> **Settings → Pages → Branch: `main` → Folder: `/ (root)` → Save**
> Na 1–2 minuten is de site bereikbaar.

## Lokaal openen

Open `index.html` in je browser, of start een lokale server:

```bash
cd "Voorstel IFJ"
python3 -m http.server 8080
```

Ga daarna naar http://localhost:8080

## Navigatie

- Pijltjestoetsen of spatiebalk — volgende slide
- Shift + pijltje links — vorige slide
- Knoppen onderaan — vorige / volgende
- Op mobiel — swipen

## Inhoud (9 slides)

1. Titel — AI Onboardingsdag
2. Aanleiding
3. De wens
4. De aanpak
5. Wie doet wat (Caesar + Max)
6. De dag
7. De output
8. Vervolgstappen
9. Tot slot

## Stijl

Zelfde opzet als het [Josh V voorstel](https://cschoorl.github.io/Josh-V-Voorstel/): Cormorant Garamond, Inter, minimalistisch fullscreen deck.
